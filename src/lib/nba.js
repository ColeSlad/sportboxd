...
function bdlHeaders() {
    if (!API_KEY)
        throw new Error('VITE_BALLDONTLIE_API_KEY is not set');
    return { Authorization: API_KEY };
}
export async function bdlFetchGames(params) {
    ...
    if (!res.ok)
        throw new Error(`BDL API ${res.status}: ${await res.text()}`);
    return res.json();
}
export async function bdlFetchGame(id) {
...
function lsGet(key) {
    try {
        ...
        if (Date.now() - ts > CACHE_TTL) {
            localStorage.removeItem(`bdl:${key}`);
            return null;
        }
        return data;
    }
...
function lsSet(key, data) {
    try {
        localStorage.setItem(`bdl:${key}`, JSON.stringify({ data, ts: Date.now() }));
    }
...
export function fetchGamesFromBDL(params) {
    ...
    if (lsCached) {
        memCache.set(cacheKey, lsCached);
        return Promise.resolve(lsCached);
    }
