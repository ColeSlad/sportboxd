...
function BoxScoreTable({ gameId, homeTeam, awayTeam }) {
    ...
    useEffect(() => {
        fetchBoxScore(gameId).then(setRows).catch((e) => setError(e.message));
    }, [gameId]);
    if (error === 'BDL_UPGRADE_REQUIRED')
        return (_jsxs("div", { className: "py-12 text-center text-gray-600", children: [_jsx("p", { className: "text-sm mb-1", children: "Box scores require a paid balldontlie.io plan." }), _jsx("p", { className: "text-[0.75rem] text-gray-700", children: "Upgrade at balldontlie.io to unlock player stats." })] }));
    if (error)
