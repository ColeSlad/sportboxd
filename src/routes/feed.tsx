  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-24">
      <div className="py-8">
        ...
        <p className="text-gray-600 text-sm">What the people you follow are watching and rating.</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        {suggested.length > 0 && (
          <aside className="lg:w-64 flex-shrink-0">
            <h3 className="font-condensed font-bold tracking-widest uppercase text-gray-500 text-[0.8rem] mb-3">Suggested</h3>
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[0.85rem] mb-1.5">
            ...
            <Link to="/games/$gameId" params={{ gameId: String(item.game.id) }}
              className="text-accent font-medium hover:brightness-125 transition-all">
              {away.name} @ {home.name}
            </Link>
          </div>
          {item.rating != null && <div className="mb-1.5"><StarRating value={item.rating} readOnly size="sm" /></div>}
          {item.excerpt && (
            <p className="text-[0.82rem] text-gray-500 leading-relaxed line-clamp-2 mb-2">"{item.excerpt}"</p>
          )}
          <div className="flex items-center gap-3">
            <span className="text-[0.7rem] text-gray-700">{formatRelativeTime(item.time)}</span>
...
function SuggestedUser({ user }: { user: AppUser }) {
  ...
  return (
    <div className="card p-3 flex items-center gap-3">
      ...
      <button className={`btn btn-sm flex-shrink-0 ${following ? 'btn-ghost' : 'btn-primary'}`}
        onClick={() => { setFollowing(f => !f); followUser(user.id, !following) }}>
        {following ? '✓' : '+'}
      </button>
    </div>
  )
