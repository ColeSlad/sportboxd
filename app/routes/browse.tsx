...
function BrowsePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-24">
          ...
          {TYPE_FILTERS.map((f) => (
            <button
              ...
              className={`badge cursor-pointer transition-all ${
                search.type === f.value || (f.value === 'all' && !search.type)
                  ? 'badge-green'
                  : 'badge-gray hover:border-white/20 hover:text-white'
              }`}
            >
              {f.label}
            </button>
        ...
        <div className="card p-4 mb-4 fade-in grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                set({ sort: e.target.value as z.infer<typeof searchSchema>['sort'] })
              }
            >
              {SORT_OPTIONS.map((o) => (
      ...
      {/* Sort strip (when filters hidden) */}
      {!showFilters && (
        <div className="flex items-center gap-3 mb-5">
          <span className="text-[0.72rem] font-condensed font-bold tracking-widest uppercase text-gray-700">
            Sort:
          </span>
      ...
      {games.length === 0 ? (
        <div className="text-center py-20 text-gray-600">
          ...
          <p className="text-sm">No games match your filters. Try broadening the search.</p>
          <button className="btn btn-ghost btn-sm mt-4" onClick={() => navigate({ search: {} })}>
            Clear filters
          </button>
        </div>
      ) : (
